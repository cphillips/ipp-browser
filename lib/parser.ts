...
export default function (buf: any) {
  ...
  function read(length: number, enc?: string | undefined) {
    if (length == 0) return "";
  ...
  function readAttr(group: { [x: string]: any; }) {
    let tag = read1();
  ...
  function readValues(type: any, name: any) {
  ...
  function readValue(tag: number, name?: any) {
    let length = read2();
  ...
  function readCollectionItemValue(name?: any) {
    let tag = read1();
    //TODO: find a test for this
    if (tag === 0x7f) {
      //tags.extension
...
export const handleUnknownTag = function log(tag: any, name: string, length: any, read: (arg0: any) => any) {
